...
void GetChannelWhitelist(uint64_t guild_id, db::callback cb)
{
	db::query("SELECT channel_id FROM channel_whitelist WHERE guild_id = '?'", {guild_id}, cb);
}
...
void command_start_t::call(const in_cmd &cmd, std::stringstream &tokens, guild_settings_t &settings, const std::string &username, bool is_moderator, dpp::channel* c, dpp::user* user)
{
	...
	GetChannelWhitelist(cmd.guild_id, [this, cmd, settings, questions, quickfire, hintless, category, c, username](db::resultset rs) {
		...
		if (!already_running) {
			...
			uint32_t ques = questions;
			if (!quickfire) {
				if (hintless) {
					if (settings.max_hardcore_round < ques) {
						ques = settings.max_hardcore_round;
						creator->SimpleEmbed(settings, ":warning:", fmt::format(_("LIMITED", settings), settings.max_hardcore_round), cmd.channel_id);
					}
				} else {
					if (settings.max_normal_round < ques) {
						ques = settings.max_normal_round;
						creator->SimpleEmbed(settings, ":warning:", fmt::format(_("LIMITED", settings), settings.max_normal_round), cmd.channel_id);
					}
				}
			} else {
				if (settings.max_quickfire_round < ques) {
					ques = settings.max_quickfire_round;
