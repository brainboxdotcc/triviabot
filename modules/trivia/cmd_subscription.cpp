...
void command_subscription_t::call(const in_cmd &cmd, std::stringstream &tokens, guild_settings_t &settings, const std::string &username, bool is_moderator, dpp::channel* c, dpp::user* user)
{
	...
	if (access.size() && user_id) {
		...
		if (user.size()) {
			db::resultset premguild = db::query("SELECT * FROM trivia_guild_cache WHERE snowflake_id = ?", {user[0]["guild_id"].getString()});
			std::vector<field_t> fields = {
				{ "Active", _((user[0]["active"].getBool() ? "TICKYES" : "CROSSNO"), settings), true },
				{ "Subscription ID", user[0]["subscription_id"].getString() + BLANK_EMOJI, true },
				{ "Payment Has Failed", _((user[0]["payment_failed"].getBool() ? "TICKYES" : "CROSSNO"), settings), true },
				{ "Plan Type", ReplaceString(user[0]["plan_id"], "triviabot-premium-", "") + BLANK_EMOJI, true },
				{ "Guild", user[0]["guild_id"].getString() + "\n" + (premguild.size() ? premguild[0]["name"].getString() : "<not assigned>"), true },
				{ "Subscribed Since", user[0]["hr_since"].getString() + BLANK_EMOJI, true },
				{ "Cancelled Date", user[0]["hr_cancel_date"].getString() + BLANK_EMOJI, true },
				{ "Last Payment Failed Date", user[0]["hr_payment_failed_date"].getString() + BLANK_EMOJI, true },
				{ "Manual Expiry Date", user[0]["hr_manual_expiry_date"].getString() + BLANK_EMOJI, true },
			};
